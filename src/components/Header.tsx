      ...
      <div className="container mx-auto px-4">
        <div className="flex h-20 items-center justify-between">
          <Link to="/" className="flex items-center space-x-3 group">
            ...
            <div>
              <h1 className="font-heading text-2xl font-bold text-primary">Papillons</h1>
              <p className="text-xs text-muted-foreground">Orphelinat</p>
            </div>
          </Link>
          ...
          <nav className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
              <Link
                key={link.to}
                to={link.to}
                className="px-4 py-2 rounded-lg text-foreground hover:bg-primary/10 hover:text-primary transition-colors duration-200 font-medium"
              >
                {link.label}
              </Link>
            ))}
